...
from data.challenge2012 import load_challenge_2012, load_c12_pretrain
from data.challenge2019 import load_challenge_2019, load_challenge_2019_pretrain
...
def random_masking(x, original_mask, min_mask_ratio, max_mask_ratio):
    ...
    x = x * (~mask)  # mask value: True for reconstruction, False for original
    return x, original_mask * (~mask),  original_mask * mask # first:maaked data second:mask(combine original and added_mask) third:use to identify where is masked(to calculate loss)
...
if __name__ == "__main__":
    ...
    if args.dataset == 'c12':
        ...
        train_dataset, val_dataset, test_dataset = load_c12_pretrain()
    elif args.dataset == 'c19':
        args.input_dim = 34
        args.demo_dim = 5
        args.num_class = 2
        args.max_len = 60
        train_dataset, val_dataset, test_dataset = load_challenge_2019_pretrain()
