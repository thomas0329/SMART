...
CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nnodes 1 --nproc_per_node 1 --master_port $random_port \
    main_pretrain.py --dataset $dataset --seed $seed > pretrain_${dataset}_${seed}.log 2>&1
CUDA_VISIBLE_DEVICES=0 python -m torch.distributed.launch --nnodes 1 --nproc_per_node 1 --master_port $random_port \
    main_finetune.py --dataset $dataset --seed $seed > finetune_${dataset}_${seed}.log 2>&1
done
done
